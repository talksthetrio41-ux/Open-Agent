...
- **LD_PRELOAD is a two-sided trap on Termux.** Kept: Chromium's crashpad/zygote re-exec of `/proc/self/exe` dies with `CANNOT LINK EXECUTABLE … not accessible for the namespace`. Stripped: some devices instead fail at the wrapper's exec with EACCES (exit 126). Never pick one — `CdpChrome._attempts()` must keep the strategy matrix (wrapper + direct `$PREFIX/lib/chromium/chrome` binary, each with and without `LD_PRELOAD`), preferring `--single-process --no-zygote --in-process-gpu` + `--disable-crash-reporter` so no re-exec happens at all. Launch failure must include `_diagnostics()` (file modes, SELinux state) and suggest `pkg reinstall chromium`.
- `install.sh` must restore the exec bit on `$PREFIX/lib/chromium/chrome` if missing (partial pkg upgrades drop it → wrapper exit 126).
