...
- **Strip `LD_PRELOAD`** when launching Chromium on Termux: `libtermux-exec.so` is not accessible inside the Android runtime linker namespace, so Chromium's re-exec of `/proc/self/exe` fails with `CANNOT LINK EXECUTABLE … not accessible for the namespace`.
